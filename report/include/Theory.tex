...
\reviewer{This chapter should contain a good number of citations.}
...
type, so it is very\reviwer{UA} redundant to specify the type in every call to the function
...
\begin{equation} \label{eq:veri2}
  s_I = s_A\ .\ tr
\end{equation}\reviewer{Use the function composition ring. A $.$ is programmer-speak for composition, not typical mathematical notation}

This is hard to show in practice however.\reviewer{Well, you don't know that, since that is not what you are investigating.} Simply defining $s_A$ or $s_I$ for
subsets of the languages is a big project. Verifying the above equation is way\reviewer{UA} beyond the scope of this
thesis.\reviewer{You should phrase things differently. As you say, a trivial transpiler is ``correct''. You also should mention how you will have any idea at all that your definitions are correct if you don't prove the above correspondence -- just defining $s_A$ and $s_I$ with no regards to correctness is easy.}
...
time, so we either need to construct a formal correctness proof or test a subset of $p$.
Developing a formal correctness proof is an huge\reviewer{UA} undertaking, there
exists only a handful of formally verified compilers published.\reviewer{2, as far as I'm aware, CakeML and CompCert (apart from toy compilers). Good place to add citations.} Therefore we
will focus on partial verification by testing, we explore our options in
section~\ref{sec:methveri}.\reviewer{Note that it's a different game to verify a compiler and a transpiler. In theory, the transpiler would have way fewer steps, which could make proving easier. Rather than talking about how very hard things are, just say it's a matter for further research. You sound defensive when you paint your limitations as set by very hard problems. It's okay to just say it is out of scope. Focus on what you are doing, and what you want to include in your thesis. If the scope is sensible, it will seem sensible to the reader (it does, a transpiler for a good subset of Idris and Agda is in itself a very interesting and decently-sized project, no one would expect much formal verification. But I would expect at least some reasoning about correctness, or the software engineering aspects, in lieu of actual verification, not just labeling it ``hard'' and moving it to the side.)}
...
been done before.\reviewer{... to the best of our knowledge.} However several translations between proof systems have been
attempted. The benefit of reusing existing proofs in a new system is obvious\reviewer{Not obvious, but you explained it in the intro.}.
